...
# --- LOB Splatter (GMM-based liquidity wall detection) ---
class LOBSplatter:
    def __init__(self, n_components=3, sigma_p=2.0):
        self.n_components = n_components
        self.sigma_p = sigma_p
        self.gmm_bid = GaussianMixture(n_components=n_components, covariance_type='diag', warm_start=True, random_state=42)
        self.gmm_ask = GaussianMixture(n_components=n_components, covariance_type='diag', warm_start=True, random_state=42)

    def _generate_samples(self, prices, volumes, num_samples=1000):
        vol_sum = np.sum(volumes)
        if vol_sum == 0:
            return np.array([])
        probs = volumes / vol_sum
        return np.random.choice(prices, size=num_samples, p=probs).reshape(-1, 1)

    def fit_snapshot(self, snapshot_df):
        results = {'bid': [], 'ask': []}
        for side in ['bid', 'ask']:
            side_df = snapshot_df[snapshot_df['side'] == side].copy()
            if side_df.empty:
                continue
            prices = side_df['price'].values
            volumes = side_df['volume'].values
            smoothed_vols = gaussian_filter1d(volumes, sigma=self.sigma_p)
            samples = self._generate_samples(prices, smoothed_vols)
            if len(samples) == 0:
                continue
            gmm = self.gmm_bid if side == 'bid' else self.gmm_ask
            gmm.fit(samples)
            for i in range(self.n_components):
                mu = gmm.means_[i][0]
                sigma = np.sqrt(gmm.covariances_[i][0])
                alpha = gmm.weights_[i] * np.sum(smoothed_vols)
                mask = (prices >= mu - sigma) & (prices <= mu + sigma)
                cluster_orders = np.sum(side_df['orders'].values[mask])
                avg_order_size = np.sum(volumes[mask]) / cluster_orders if cluster_orders > 0 else 0
                results[side].append({'cluster_id': i, 'mu': mu, 'sigma': sigma,
                                      'alpha': alpha, 'avg_order_size': avg_order_size})
        return results

    def process_session(self, df, progress_bar=None):
        ...
        for i, t in enumerate(timestamps):
            ...
            mid_price = (bids.max() + asks.min()) / 2 if not bids.empty and not asks.empty else np.nan
            for side, clusters in self.fit_snapshot(snap).items():
                for c in clusters:
                    history.append({'timestamp': t, 'side': side, 'mid_price': mid_price, **c})
            if progress_bar is not None:
                progress_bar.progress((i + 1) / len(timestamps), text=f"Fitting GMMs… {i+1}/{len(timestamps)}")
        history_df = pd.DataFrame(history)
        if history_df.empty:
            return history_df
        history_df = history_df.sort_values('timestamp')
        for col in ['mu', 'sigma', 'alpha']:
            history_df[f'delta_{col}'] = history_df.groupby(['side', 'cluster_id'])[col].diff()
        roll_win = 360 * self.n_components
        for col, agg in [('sigma', 'median'), ('alpha', lambda x: x.quantile(0.60)),
                         ('avg_order_size', lambda x: x.quantile(0.90))]:
            key = {'sigma': 'roll_sigma_median', 'alpha': 'roll_alpha_high',
                   'avg_order_size': 'roll_whale_size'}[col]
            history_df[key] = history_df.groupby('side')[col].transform(
                lambda x: x.rolling(roll_win, min_periods=30).agg(agg))
        history_df['roll_mu_shift'] = history_df.groupby('side')['delta_mu'].transform(
            lambda x: x.abs().rolling(roll_win, min_periods=30).quantile(0.75))
        return history_df
    ...
    def classify_states(self, df):
        ...
        prev_states, state_labels = {}, []
        for _, row in df.iterrows():
            key = (row['side'], row['cluster_id'])
            prev = prev_states.get(key, 'Initializing')
            if pd.isna(row['delta_mu']) or pd.isna(row['roll_sigma_median']):
                state = 'Initializing'
            else:
                SIG_LO = row['roll_sigma_median'] * 1.15
                A_HI = row['roll_alpha_high']
                is_whale = row['avg_order_size'] > row['roll_whale_size']
                if row['sigma'] > row['roll_sigma_median'] * 4.0:
                    state = 'Sparse/Fragmented'
                elif row['delta_alpha'] < -(A_HI * 0.2):
                    state = 'Evaporating Wall'
                elif row['alpha'] > A_HI and abs(row['delta_mu']) > row['roll_mu_shift']:
                    state = 'Migrating Wall'
                elif 'Rigid Wall' in prev:
                    state = ('Rigid Wall (Whale)' if is_whale else 'Rigid Wall') \
                        if row['sigma'] < SIG_LO * 1.10 and row['alpha'] > A_HI * 0.90 \
                        else 'Standard Depth'
                else:
                    state = ('Rigid Wall (Whale)' if is_whale else 'Rigid Wall') \
                        if row['sigma'] < SIG_LO and row['alpha'] > A_HI and abs(row['delta_sigma']) < 0.1 \
                        else 'Standard Depth'
    ...
    def detect_squeezes(self, df):
        squeeze_times = []
        for t, grp in df.groupby('timestamp'):
            bids, asks = grp[grp['side'] == 'bid'], grp[grp['side'] == 'ask']
            if ((any((bids['state_label'] == 'Migrating Wall') & (bids['delta_mu'] > 0)) and
                 any(asks['state_label'].str.contains('Rigid Wall'))) or
                (any((asks['state_label'] == 'Migrating Wall') & (asks['delta_mu'] < 0)) and
                 any(bids['state_label'].str.contains('Rigid Wall')))):
                squeeze_times.append(t)
        df['squeeze_alert'] = df['timestamp'].isin(squeeze_times)
        return df


def prepare_depth_for_lob(df_depth):
    """Adapts the loaded df_depth DataFrame into the format LOBSplatter expects."""
    df = df_depth[['datetime', 'Price', 'Volume', 'Number_of_Orders', 'Type']].copy()
    df = df.rename(columns={'datetime': 'timestamp', 'Price': 'price',
                             'Volume': 'volume', 'Number_of_Orders': 'orders'})
    df['side'] = df['Type'].map({'BUY': 'bid', 'SELL': 'ask'})
    mask = (df['timestamp'].dt.time >= datetime.time(10, 0)) & \
           (df['timestamp'].dt.time <= datetime.time(16, 0))
    return df[mask][['timestamp', 'price', 'volume', 'orders', 'side']].dropna(subset=['side']).sort_values(
        ['timestamp', 'price'])
...
if analysis_type == "Market Depth Explorer":
    ...
    if df_depth is not None and not df_depth.empty and df_sales is not None and not df_sales.empty:
        ...
        # ==========================================
        # 0. ORDER BOOK REPLAY (Collapsible)
        # ==========================================
        with st.expander("🎬 Order Book Replay", expanded=False):
            st.markdown("GMM-based liquidity wall detection: identifies, classifies and tracks bid/ask walls through the full session.")

            n_snaps = df_depth['datetime'].nunique()
            est_lo, est_hi = max(1, n_snaps // 300), max(2, n_snaps // 150)
            col_run, col_info = st.columns([1, 3])
            with col_run:
                run_lob = st.button("▶️ Run LOB Analysis", help="Fits GMMs across all snapshots. Result cached for the session.")
            with col_info:
                st.caption(f"~{n_snaps:,} snapshots detected. Estimated processing time: {est_lo}–{est_hi} min.")

            if run_lob:
                lob_prep = prepare_depth_for_lob(df_depth)
                splatter = LOBSplatter(n_components=3, sigma_p=2.0)
                prog = st.progress(0, text="Fitting GMMs…")
                states = splatter.process_session(lob_prep, progress_bar=prog)
                prog.progress(1.0, text="Classifying wall states…")
                states = splatter.classify_states(states)
                states = splatter.detect_squeezes(states)
                prog.empty()
                st.session_state['lob_states'] = states
                st.session_state['lob_raw'] = lob_prep
                st.success(f"Analysis complete — {n_snaps:,} snapshots processed.")

            if st.session_state.get('lob_states') is not None:
                lob_states = st.session_state['lob_states']
                lob_raw = st.session_state['lob_raw']

                # Summary metrics
                state_counts = lob_states['state_label'].value_counts()
                squeeze_count = int(lob_states[lob_states['squeeze_alert']]['timestamp'].nunique())
                metric_cols = st.columns(min(len(state_counts) + 1, 6))
                for i, (label, cnt) in enumerate(state_counts.items()):
                    if i < 5:
                        metric_cols[i].metric(label, int(cnt))
                metric_cols[-1].metric("⚠️ Squeeze Alerts", squeeze_count)

                # Full-session mid-price + squeeze timeline
                mid_prices = (
                    lob_raw.groupby('timestamp')
                    .apply(lambda g: (g[g['side'] == 'bid']['price'].max() +
                                      g[g['side'] == 'ask']['price'].min()) / 2
                           if not g[g['side'] == 'bid'].empty and not g[g['side'] == 'ask'].empty
                           else np.nan)
                    .reset_index(name='mid_price')
                )
                squeeze_times = lob_states[lob_states['squeeze_alert']]['timestamp'].unique()
                fig_tl = go.Figure()
                fig_tl.add_trace(go.Scatter(
                    x=mid_prices['timestamp'], y=mid_prices['mid_price'],
                    mode='lines', name='Mid Price', line=dict(color='steelblue', width=1.5)
                ))
                if len(squeeze_times) > 0:
                    sq_mids = mid_prices[mid_prices['timestamp'].isin(squeeze_times)]
                    fig_tl.add_trace(go.Scatter(
                        x=sq_mids['timestamp'], y=sq_mids['mid_price'],
                        mode='markers', name='Squeeze Alert',
                        marker=dict(color='purple', size=8, symbol='triangle-up')
                    ))
                fig_tl.update_layout(
                    height=250, title='Mid Price + Squeeze Alerts (Full Session)',
                    margin=dict(l=40, r=40, t=40, b=30),
                    legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1)
                )
                st.plotly_chart(fig_tl, use_container_width=True)

                # Per-snapshot scrubber
                st.markdown("**Snapshot Viewer**")
                all_times = sorted(lob_raw['timestamp'].unique())
                sel_time = st.select_slider(
                    "Scrub to Snapshot", options=all_times,
                    format_func=lambda t: pd.Timestamp(t).strftime('%H:%M:%S'),
                    key="lob_snapshot_slider"
                )
                snap_raw = lob_raw[lob_raw['timestamp'] == sel_time]
                snap_states = lob_states[lob_states['timestamp'] == sel_time]
                bid_raw = snap_raw[snap_raw['side'] == 'bid']
                ask_raw = snap_raw[snap_raw['side'] == 'ask']

                fig_snap = go.Figure()
                if not bid_raw.empty:
                    fig_snap.add_trace(go.Bar(x=bid_raw['price'], y=bid_raw['volume'],
                                              name='Bid', marker_color='rgba(0,180,0,0.4)'))
                if not ask_raw.empty:
                    fig_snap.add_trace(go.Bar(x=ask_raw['price'], y=ask_raw['volume'],
                                              name='Ask', marker_color='rgba(220,0,0,0.4)'))

                state_colors = {
                    'Rigid Wall': 'darkgreen', 'Rigid Wall (Whale)': 'goldenrod',
                    'Migrating Wall': 'orange', 'Evaporating Wall': 'crimson',
                    'Sparse/Fragmented': 'grey', 'Standard Depth': 'steelblue', 'Initializing': 'lightgrey'
                }
                for _, row in snap_states.iterrows():
                    fig_snap.add_vline(
                        x=row['mu'], line_color=state_colors.get(row['state_label'], 'black'),
                        line_dash='dash', line_width=1.5,
                        annotation_text=f"{row['state_label']}<br>σ={row['sigma']:.2f}",
                        annotation_font_size=9
                    )

                is_squeeze = not snap_states.empty and snap_states['squeeze_alert'].iloc[0]
                snap_title = f"Order Book @ {pd.Timestamp(sel_time).strftime('%H:%M:%S')}"
                if is_squeeze:
                    snap_title += "  ⚠️ SQUEEZE"
                if not bid_raw.empty and not ask_raw.empty:
                    mid = (bid_raw['price'].max() + ask_raw['price'].min()) / 2
                    fig_snap.update_xaxes(range=[mid - 30, mid + 30])
                fig_snap.update_layout(
                    title=snap_title, xaxis_title='Price', yaxis_title='Volume',
                ...
                # Download button
                csv_bytes = lob_states.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="⬇️ Download LOB States CSV",
                    data=csv_bytes,
                    file_name=f"LOB_States_{date_str}.csv",
                    mime="text/csv"
                )
